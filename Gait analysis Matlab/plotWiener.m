...
% periodogram(x,rectwin(length(x)),length(x),Fs) for reference

[Pm, Fm] = periodogram(model,hamming(length(model)),length(model),Fs);
[Pn, Fn] = periodogram(noise,hamming(length(noise)),length(noise),Fs);
[Psig, Fsig] = periodogram(observation,hamming(length(observation)),length(observation),Fs);
[Pw, Fw] = periodogram(utilda,hamming(length(utilda)),length(utilda),Fs);

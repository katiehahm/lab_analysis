...
starts_trainX = dataM[starts_train_idx,3:15] # the rest of the array is just 0 filler
    ...
    for n in range(np.size(test_idx)):
        ...
        if n != np.size(test_idx)-1: # not the last element
            if (test_idx[0,n+1]-test_idx[0,n]) == 1: # continuation of same walking cycle
                ...
                xTest[n+1] = np.append(nextFVector[0:20],yPredict) # use current predicted value on next impact FVector

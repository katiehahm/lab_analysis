...
def RFtest(inp, out, k_fold):
    ...
    for k, (train, test) in enumerate(k_fold.split(inp, out)):
        ...
        param_dist = {'n_estimators' : 200, 'min_samples_leaf' : 1, 'min_samples_split' : 2, 'min_weight_fraction_leaf' : 0}

        model_rf = RandomForestRegressor()
        model_rf.set_params(**param_dist)
        regr_multi = MultiOutputRegressor(model_rf)

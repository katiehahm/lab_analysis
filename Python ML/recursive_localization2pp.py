...
starts_trainX = dataM[starts_train_idx,2:25] # the rest of the array is just 0 filler
starts_trainY = dataM[starts_train_idx,1]
...
for i in range(5):
    ...
    xTest = dataM[test_idx,2:]
    yTest = dataM[test_idx,1]
    xTest = xTest[0,:,:]
    yTest = yTest.flatten()
    ...
    for n in range(np.size(test_idx)):
        ...
        if n != np.size(test_idx)-1: # not the last element
            if (test_idx[0,n+1]-test_idx[0,n]) == 1: # continuation of same walking cycle
                ...
                xTest[n+1] = np.append(nextFVector[0:42],yPredict) # use current predicted value on next impact FVector
...
DF.to_csv("both_towards1_person1_results.csv") # CHANGE ***********************************
